...
<body>
	...
	<div class="pd-20" style="width: 80%">
			...
			<div class="row cl">
			<input type="hidden" id="area_id" value="${list.area_id}">
						<label class="form-label col-2">名称：</label>
						<div class="formControls col-10">
							<input type="text" id="area_name"
								placeholder="请填写区域名称" value="${list.area_name}" class="input-text" style="width: 80%">
						</div>
					</div></c:forEach>
					<br><br><br>
	...
	function add(){
		...
		$.ajax({
			url:'areaUpdate',
			type:'post',
			data:'area_name='+encodeURI(area_name)+'&area_id='+area_id,
			success:function(rs){
				if(rs==1){
					alert("成功！");
