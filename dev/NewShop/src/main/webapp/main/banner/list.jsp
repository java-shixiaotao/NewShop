...
<body>
	<nav class="breadcrumb">
		...
		图片查看 <a class="btn btn-success radius r mr-20"
				...
				<tbody>
					<c:forEach items="${list}" var="list" varStatus="s">
						<tr class="text-c">
							...
							class="input-text" onblur="sort('${list.ban_id}','sort${s.index}')"></td>
							<td>${list.url}</td>
							<td>
							<c:if test="${list.type==1}">轮动</c:if>
							<c:if test="${list.type==2}">广告</c:if>
							</td>
